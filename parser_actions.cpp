...
{
	TokenValue t;
	t.parameter_list=new vector<pair<string,shared_ptr<TypeAST> > >();
	return t;
}
...
{
	TokenValue t;
	t.parameter_list=new vector<pair<string,shared_ptr<TypeAST> > >(*values[0].parameter_list);
...
{
	TokenValue t;
	t.parameter_ast=new pair<string,shared_ptr<TypeAST> >(values[0].str,values[2].type_ast);
	return t;
}
...
{
	TokenValue t;
	t.variabledef_list=new vector<shared_ptr<VariableDefStatementAST> >();
...
{
	TokenValue t;
	t.variabledef_list=new vector<shared_ptr<VariableDefStatementAST> >(*values[0].variabledef_list);
...
{
	TokenValue t;
	t.variabledef_ast=new VariableDefStatementAST(new pair<string,shared_ptr<TypeAST> >(values[0].str,values[2].type_ast),NULL);
	return t;
}
...
{
	TokenValue t;
	t.variabledef_ast=new VariableDefStatementAST(new pair<string,shared_ptr<TypeAST> >(values[0].str,values[2].type_ast),new UnBuiltExprAST(values[4].expression_list));
	return t;
}

TokenValue variabledef_infer_reduce(shared_ptr<CodegenInfo>,vector<TokenValue> values){
	TokenValue t;
	t.variabledef_ast=new VariableDefStatementAST(new pair<string,shared_ptr<TypeAST> >(values[0].str,NULL),new UnBuiltExprAST(values[2].expression_list));
	return t;
}
...
{
	TokenValue t;
	t.statement_list=new vector<shared_ptr<StatementAST> >();
	return t;
}
...
{
	TokenValue t;
	t.statement_list=new vector<shared_ptr<StatementAST> >(*values[0].statement_list);
...
{
	TokenValue t;
	t.expression_list=new vector< shared_ptr<ExprAST> >();
...
{
	TokenValue t;
	t.expression_list=new vector< shared_ptr<ExprAST> >(*values[0].expression_list);
...
{
	TokenValue t;
	vector<shared_ptr<TypeAST> > *v=new vector<shared_ptr<TypeAST> >(*values[2].type_list);
...
{
	TokenValue t;
	t.type_list=new vector<shared_ptr<TypeAST> >();
	return t;
}
...
{
	TokenValue t;
	t.type_list=new vector<shared_ptr<TypeAST> >(*values[0].type_list);
...
{
	TokenValue t;
	t.arg_exp_list=new vector<shared_ptr<ExprAST> >();
	return t;
}
...
{
	TokenValue t;
	t.arg_exp_list=new vector<shared_ptr<ExprAST> >(*values[0].expression_list);
...
{
	TokenValue t;
	t.statement_ast=new IfStatementAST(new UnBuiltExprAST(values[2].expression_list),values[5].block_ast,new BlockAST(new vector<shared_ptr<StatementAST> >()));
	return t;
}
...
TokenValue listvalexpr_reduce(shared_ptr<CodegenInfo> cgi,vector<TokenValue> values){
	TokenValue t;
	list<shared_ptr<ExprAST>> *lst=new list<shared_ptr<ExprAST>>();
...
TokenValue gmlist_addvoid_reduce(shared_ptr<CodegenInfo> cgi, vector<TokenValue> values)
{
	...
	values[0].groupdef_ast->MemberList.push_back(pair<string,shared_ptr<TypeAST> >(values[2].str,new FunctionTypeAST(args)));
...
{
	TokenValue t;
	t.datainitval_list=new vector< pair<string,shared_ptr<ExprAST> > >(); //parameterと型が一緒なので使わせてもらう
	return t;
}
...
{
	TokenValue t;
	vector< pair<string,shared_ptr<ExprAST> > > *m=new vector< pair<string,shared_ptr<ExprAST> > >(*(values[0].datainitval_list)); //parameterと型が一緒なので使わせてもらう
	m->push_back(pair<string,shared_ptr<ExprAST>>(values[1].str,new UnBuiltExprAST(values[3].expression_list)));
