...
public:
    ...
    shared_ptr<vector<label_or_immediate> > CodePtr; //バイトコードへのポインタ
    shared_ptr<Flame> StaticLink;
    shared_ptr<Flame> DynamicLink;
	shared_ptr<FunctionObject> FunctionInfo;
    int PC;
    Flame(shared_ptr< vector< pair<string,VMValue> > > vars,shared_ptr< vector<label_or_immediate> > codeptr,shared_ptr<Flame> dynamiclink,shared_ptr<Flame> staticlink,shared_ptr<FunctionObject> finfo)
		:Variables(vars),CodePtr(codeptr),DynamicLink(dynamiclink),StaticLink(staticlink),FunctionInfo(finfo){
    	PC=0;
...
public:
    ...
    static void BytecodeToLabels(shared_ptr< vector<int> >,shared_ptr< vector<label_or_immediate> >);
};
